...
def get_associations():
    assocs = Association.query.options(FromCache(cache)).order_by(Association.order).all()
    ret = [{'id': assoc.id, 'speech_id': assoc.speech_id, 'action_id': assoc.action_id, 'order': assoc.order} for assoc
           in assocs]
...
def set_association():
    if request.method == 'POST':
        ...
        if id is None or id == 0:
            ...
            assoc = Association.query.options(FromCache(cache)).get(id)
            assoc.speech_id = request.json['speech_id']
            assoc.action_id = request.json['action_id']
            assoc.order = request.json['order']
            db.session.commit()
        return flask.jsonify(
            {'id': assoc.id, 'speech_id': assoc.speech_id, 'action_id': assoc.action_id, 'order': assoc.order})
    else:
        return flask.jsonify({'result': 'nothing to show here!'})
...
def delete_association():
    if request.method == 'POST':
        ...
        prev = 0
        associations = Association.query.options(FromCache(cache)).order_by(Association.order).all()
        ...
        ret = [{'id': assc.id, 'speech_id': assc.speech_id, 'action_id': assc.action_id, 'order': assc.order} for assc
               in associations]
...
def reorder_associations():
    if request.method == 'POST':
        ...
        ret = [{'id': assoc.id, 'speech_id': assoc.speech_id, 'action_id': assoc.action_id, 'order': assoc.order} for
               assoc in assocs]
        return flask.jsonify({'result': 'success', 'retrows': ret})
    else:
        return flask.jsonify({'result': 'error'})
